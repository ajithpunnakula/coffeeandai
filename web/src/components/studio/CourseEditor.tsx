...
export default function CourseEditor({ initialCourse }: CourseEditorProps) {
    ...
    try {
      ...
      await Promise.all(
        cardIds.map(async (cardId) => {
          ...
          if (!card) return;
          const res = await fetch(`/api/studio/cards/${cardId}`, {
            method: "PATCH",
      ...
      if (dirtyCourse.current) {
        ...
        const res = await fetch(`/api/studio/courses/${currentCourse.slug}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function handleAddCard(type: string) {
    ...
    await fetch("/api/studio/cards/reorder", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        courseSlug: course.slug,
  ...
  async function handlePublish() {
    ...
    try {
      ...
      const res = await fetch(`/api/studio/courses/${course.slug}/publish`, {
        method: "POST",
      });
      if (!res.ok) {
