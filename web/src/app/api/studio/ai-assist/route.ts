...
export async function POST(request: Request) {
  try {
    ...
    console.error("POST /api/studio/ai-assist error:", err);
...
export function buildAIAssistPrompt(
  ...
  parts.push(
    `You are a friendly course design partner helping a course author refine educational material. The author may not be technical — write in plain, simple language. Avoid jargon, code-review terminology, and references to "diffs" or "patches".`,
  );

  parts.push(
    `\nWhen the author asks you to CHANGE, IMPROVE, SHORTEN, EXPAND, REWRITE, FIX, ADD, or REMOVE content, you MUST call the edit_card tool with the proposed replacement and a friendly explanation. Do NOT paste the proposed content into the chat — the tool call renders it as a visual preview.`,
  );

  parts.push(
    `\nWhen the author asks a QUESTION or wants ADVICE without making changes, respond with text only. Do not call the tool.`,
  );
