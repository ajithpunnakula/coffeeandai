...
export const metadata: Metadata = {
  title: "Browse",
  description:
    "Browse interactive, card-based courses and learning paths to prepare for AI certifications.",
  openGraph: {
    type: "website",
    title: "Browse",
    description:
      "Browse interactive, card-based courses and learning paths to prepare for AI certifications.",
    url: "https://coffeeandai.xyz/browse",
  },
};
...
function levelLabel(l: CatalogLevel) {
  if (!l) return null;
...
export default async function BrowsePage() {
  ...
  const courses = await sql`
    SELECT c.id, c.slug, c.title, c.summary, c.target_audience, c.estimated_minutes,
           c.pass_threshold, c.domains, c.level, c.topic_key,
  ...
  const catalogCourses: CatalogCourse[] = courses.map((c: any) => ({
    ...
    pass_threshold: c.pass_threshold ?? null,
    domains:
