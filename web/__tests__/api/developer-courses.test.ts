...
import { AuthError, requireDeveloper } from "@/lib/auth-guards";
...
const mockRequireDeveloper = vi.mocked(requireDeveloper);
...
describe("Developer courses API helpers", () => {
  ...
  describe("Auth guard integration", () => {
    it("requireDeveloper rejects learner role", async () => {
      mockRequireDeveloper.mockRejectedValue(
        new AuthError("Developer access required", 403),
      );

      await expect(requireDeveloper()).rejects.toThrow("Developer access required");
    });

    it("requireDeveloper accepts developer role", async () => {
